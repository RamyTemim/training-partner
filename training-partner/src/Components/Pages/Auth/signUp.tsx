...
const PageSignUp : React.FC = () => {
    const [signUp ,setSignUp] = useState<signUp>({ pseudo : '', nom : '', prenom : '',datedenaissance : new Date(), email : '', message_mdp : '', reponse_message : '', motdepasse : ''});
    const [error, setError] = useState<string>('');

    const handleInputChange = (event : React.ChangeEvent<HTMLInputElement>) : void => {
        const { name, value } = event.target;
        if (name === "datedenaissance" ){
            setSignUp((signUp)=>({...signUp, [name] : new Date(value)}))
        }
        else {
        ...
        <div>
            <div className='FormulaireAuth'>
                <form onSubmit = {handleSubmitSignUp} >
                    ...
                    <input type="date" id='datedenaissance' name='datedenaissance' value = {signUp.datedenaissance.toISOString().slice(0,10)} onChange={handleInputChange}/>
